...
class IntegrationTests:
    ...
    def run_tests(config: GraphAPIConfig) -> None:
        ...
        IntegrationTests.create_update_delete_group(
            api_wrapper=graph_api_wrapper, automation_app_name=automation_app_name, group_name=group_name
        )
        ...
        IntegrationTests.assign_unassign_group_to_app(
            ...
            automation_app_name=automation_app_name,
    ...
    def create_update_delete_group(
        ...
        myapp_resp = api_wrapper.applications.get_by_application_name(app_name=automation_app_name)
        owner_app_id = myapp_resp["appId"]
        myapp_sp = api_wrapper.applications.get_service_principal_by_app_id(owner_app_id)
        owner_sp_id = myapp_sp["id"]
        owner_url = api_wrapper.applications.config.api_url + "/servicePrincipals/" + owner_sp_id
    ...
    def assign_unassign_group_to_app(
        ...
        # Create
        group_id = api_wrapper.groups.create(
            group_name=group_name, owners=[owner_url], group_type=Groups.GroupType.SECURITY, with_stabilization=True
        )
    ...
    def add_remove_list_members_to_group(api_wrapper: GraphAPI) -> None:
        # List members
        group_info = api_wrapper.groups.get_by_name(group_name="Retail")
    ...
    def limit_testing_for_groups(
        ...
        for i in range(0, total_groups):
            ...
            gid = api_wrapper.groups.create(
                group_name + str(i), Groups.GroupType.SECURITY, [owner_url], with_stabilization=False
            )
